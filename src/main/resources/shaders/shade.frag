...
const vec2 heightScale = vec2(0.04, -0.02);
...
void main() {
    ...
    if (colorMode == COLOR_MODE_POSITION_ONLY) {
        ...
        height = height * heightScale.x + heightScale.y;
    ...
    if (computeLightInFS) {
        vec3 normal = normalize(texture(chosenTextureNormal, texCoord).xyz * 2.0 - 1.0);
    ...
    float attenuation;
    if (spotlight) {
        float theta = acos(dot(-spotlightDirection, normalize(spotlightAngleDirection)));
